...
def main(dataframe, normalize=False):
    """main function to train the autoencoder and detect outliers

    Args:
        dataframe (pandas): data
        normalize (bool, optional): Defaults to False. Used to weight the loss inversely
        proportional to the standard deviation of the continuous variables
    Returns:
        numpy array: 0 for inliers, -1 for outliers
    """
    ...
    # the training will be done on CPU
    ...
    if normalize == True:
        ...
        criterion = models.Autoencoder_Loss_Prob_Normalized(
        ...
        criterion = models.Autoencoder_Loss_Prob(
            binary_indices=binary_indices,
            continuous_indices=continuous_indices,
        )
    ...
    distances = [
        criterion(
            data_tensor[i, :].unsqueeze(0), model(data_tensor)[i, :].unsqueeze(0)
        ).item()
    ...
    outlier_index = [-1 if i > treshold else 0 for i in distances]
