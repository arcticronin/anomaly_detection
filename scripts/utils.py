...
def plot_TSNE_2(df=None, labels=None, dist_matrix=None):
    """Plot t-SNE visualization of dataset with labels in a color gradient
      corresponding to the probability of an observation being an outlier.

    Args:
        df (numpy matrix of shape N x T, optional):
             dataframe. Defaults to None.
        labels (numpy array of size T, optional):
            array of labels, default will be generated as np.ones(N)
            Defaults to None.
        dist_matrix (numpy matrix N x N, optional):
            sqaure pairwise distance matrix. Defaults to None.
    """
    ...
    if dist_matrix is not None:
        ...
        tsne = TSNE(
            n_components=2, verbose=1, perplexity=30, n_iter=1000, random_state=42
        )
...
def sigmoid_to_prob(k=6) -> callable:
    """closure parametrized by a k, number of methods

    Args:
        k (int, optional): number of different classes. Defaults to 6.

    Returns:
        callable: sigmoid-like function that takes number of methods that classify a single instance
    ...
    def inner(x) -> float:
        result = (
            (
                (
                    (k * (1 + np.exp(-h)) * (1 + np.exp(h)))
                    / (1 + np.exp(h) - (1 + np.exp(-h)))
                )
                / (1 + np.exp(-x + h))
            )
            - k * (1 + np.exp(-h)) / (1 + np.exp(h) - (1 + np.exp(-h)))
        ) / k
...
def jaccard_index(anom_label=-1) -> callable:
    ...
    Args:
        anom_label (int, optional): _description_. Defaults to -1.

    Returns:
        callable: function that takes 2 lists of labels and compares the prediction of the anomalies using
    ...
    def inner(l1, l2) -> float:
        ...
        if not union:
            return (
                1.0 if not intersection else 0.0
            )  # 1 if both are empty, else 0 if only one is empty
...
def plot_PCA(df, labels=None):
    ...
    plt.figure(figsize=(8, 6))
    sns.scatterplot(
        data=pca_df, x="PC1", y="PC2", hue="labels", palette="Set2", alpha=0.5
    )
...
def gower_loss(
    x_original, x_reconstructed, binary_indices, continuous_indices
) -> float:
    ...
    x_bin_reconstructed = torch.sigmoid(
        x_reconstructed[:, binary_indices]
    )  # Use sigmoid to squash outputs
    ...
    continuous_indices = [
        i for i in range(x_original.shape[1]) if i not in binary_indices
    ]
...
def plot_3d_TSNE(df=None, labels=None, dist_matrix=None):
    ...
    if labels is None:
        try:
            ...
            labels = np.ones(len(dist_matrix))
